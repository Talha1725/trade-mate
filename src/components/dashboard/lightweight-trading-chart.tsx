...
const VWAP_BAND_COLORS = ["#2962FF", "#089981", "#F23645"];
...
export function LightweightTradingChart({
  ...
  const latestVwapPoint = React.useMemo(() => {
    if (!enabledIndicators.includes("vwap")) return null;
    const points = calculateVwap(displayCandles, vwapSettings);
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      vwapUpperSeriesRefs.current = [];
      vwapLowerSeriesRefs.current = [];
  ...
  React.useEffect(() => {
    ...
    const vwapTrack = enabledIndicators.includes("vwap")
      ? calculateVwap(displayCandles, vwapSettings)
      : [];
    ...
    vwapUpperSeries.forEach((series, index) => {
      series?.setData(vwapTrack.flatMap((point) => point.upperBands[index] == null ? [] : [{ time: toSeriesTime(point.time), value: point.upperBands[index] as number }]));
    });
    vwapLowerSeries.forEach((series, index) => {
      series?.setData(vwapTrack.flatMap((point) => point.lowerBands[index] == null ? [] : [{ time: toSeriesTime(point.time), value: point.lowerBands[index] as number }]));
    });
    ...
    vwapUpperSeriesRefs.current = vwapUpperSeries;
    vwapLowerSeriesRefs.current = vwapLowerSeries;
  ...
  }, [chartDataKey, displayCompareCandles, enabledIndicators, indicatorPeriods, normalizedCompareSymbol, displayCandles, vwapSettings]);
  ...
  React.useEffect(() => {
    ...
    const vwap = calculateVwap(merged, vwapSettings);
    ...
    vwapUpperSeriesRefs.current.forEach((series, index) => {
    ...
    vwapLowerSeriesRefs.current.forEach((series, index) => {
  ...
  }, [candles, effectiveLiveQuote, indicatorPeriods, timeframe, vwapSettings]);
